...
export default function QuestionBank() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      {/* Header */}
      <div className="flex justify-between items-center bg-white p-8 rounded-[2rem] shadow-sm border border-cream-border">
        <div>
          <h1 className="text-3xl font-extrabold text-ink tracking-tight">Kho Bài Tập 📚</h1>
          <p className="text-ink-muted mt-2 font-medium">Quản lý và giao đề bài cho các bé</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-1 space-y-3">
          <h3 className="font-bold text-ink-muted uppercase tracking-wider text-sm px-4 mb-4">Các môn học</h3>
            ...
            <button
              ...
              className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all ${
                selectedSubject?.id === subject.id 
                  ? 'bg-primary text-white shadow-lg shadow-terracotta-100' 
                  : 'bg-white text-ink-muted hover:bg-cream border border-cream-border'
              }`}
            >
        ...
        <div className="md:col-span-3 space-y-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="px-4 py-2 bg-cream text-ink hover:bg-cream-border rounded-xl font-medium flex items-center gap-2 transition-colors"
              >
          ...
          {isCreatingTopic && (
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-cream-border flex gap-4 items-end animate-in fade-in slide-in-from-top-4">
              <div className="flex-1">
                <label className="block text-sm font-medium text-ink mb-1">Tên chủ đề</label>
                <input 
                  type="text" 
                  value={newTopicName}
                  onChange={e => setNewTopicName(e.target.value)}
                  className="w-full px-4 py-2 rounded-xl border border-cream-border focus:ring-2 focus:ring-primary"
                  placeholder="Ví dụ: Phép cộng phạm vi 10"
                />
              </div>
              <div className="w-32">
                <label className="block text-sm font-medium text-ink mb-1">Dành cho lớp</label>
                <select 
                  value={newTopicGrade}
                  onChange={e => setNewTopicGrade(e.target.value)}
                  className="w-full px-4 py-2 rounded-xl border border-cream-border focus:ring-2 focus:ring-primary bg-white"
                >
              ...
              <div className="flex gap-2">
                <button onClick={() => setIsCreatingTopic(false)} className="px-4 py-2 border border-cream-border rounded-xl text-ink-muted hover:bg-cream">Hủy</button>
          ...
          {filteredTopics.length === 0 ? (
            <div className="bg-white p-12 rounded-3xl border-2 border-dashed border-cream-border text-center">
              <div className="text-5xl mb-4 opacity-50">📂</div>
              <p className="text-ink-muted font-medium">Chưa có chủ đề nào cho môn học này.</p>
            </div>
          ) : (
                  ...
                  <div key={topic.id} className="bg-white rounded-3xl border border-cream-border shadow-sm overflow-hidden flex flex-col">
                    <div className="p-6 flex items-center justify-between bg-cream/50 group border-b border-cream-border shrink-0">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="font-bold text-lg text-ink">{topic.name}</h3>
                          <p className="text-sm text-ink-muted font-medium">{topic.grade}</p>
                        </div>
                      </div>
                    ...
                    <div className="p-4 bg-white flex-1">
                      {topicExams.length === 0 ? (
                        <p className="text-sm text-ink-muted italic text-center py-4">Chưa có đề thi nào trong chủ đề này. Hãy nhấn "Tạo Đề Bài" để bắt đầu.</p>
                      ) : (
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                          {topicExams.map(exam => (
                            <div key={exam.id} className="border border-cream-border p-4 rounded-2xl hover:border-primary transition-colors flex flex-col justify-between">
                              <div>
                                <div className="flex justify-between items-start mb-2">
                                  <h4 className="font-bold text-ink">{exam.name}</h4>
                                  <div className="flex gap-2">
                                    <button onClick={() => handleEditExam(exam.id, topic)} className="text-ink-muted hover:text-primary transition-colors">
                                      <Edit2 size={16} />
                                    </button>
                                    <button onClick={() => handleDeleteExam(exam.id)} className="text-ink-muted hover:text-danger transition-colors">
                                      <Trash2 size={16} />
                                    </button>
                                  </div>
                                </div>
