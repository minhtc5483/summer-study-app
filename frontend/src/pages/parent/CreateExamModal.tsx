  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-5xl max-h-[95vh] flex flex-col overflow-hidden">
        <div className="p-6 bg-gradient-to-br from-primary to-primary-dark text-white flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-2xl font-bold">{initialData ? 'Sửa Đề Thi' : 'Tạo Đề Thi Mới'}</h2>
            <p className="text-terracotta-100 mt-1">Chủ đề: {topicName}</p>
          </div>
        ...
        <div className="p-6 flex flex-col md:flex-row gap-6 flex-1 overflow-hidden">
          ...
          <div className="w-full md:w-1/3 flex flex-col gap-6 overflow-y-auto pr-2">
            <div>
              <label className="block text-sm font-medium text-ink mb-2">Tên Đề Bài</label>
              <input
                type="text"
                value={examName}
                onChange={e => setExamName(e.target.value)}
                placeholder="Ví dụ: Ôn tập cuối tuần 1"
                className="w-full px-4 py-3 rounded-xl border border-cream-border focus:ring-2 focus:ring-primary shadow-sm"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-ink mb-2">Giới hạn thời gian</label>
              <select 
                value={timeLimit === null ? '' : timeLimit.toString()} 
                onChange={e => setTimeLimit(e.target.value === '' ? null : parseInt(e.target.value))}
                className="w-full px-4 py-3 rounded-xl border border-cream-border focus:ring-2 focus:ring-primary shadow-sm bg-white"
              >
            ...
            <div>
              <label className="block text-sm font-medium text-ink mb-2">Hạn chót (Tùy chọn)</label>
              <input
                type="date"
                value={dueDate}
            ...
            <div className="bg-cream p-4 rounded-2xl border border-cream-border">
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm font-bold text-ink flex items-center gap-2">
                  <Users size={16} /> Giao bài cho bé
                </label>
                <span className="text-xs font-semibold px-2 py-1 bg-white border border-cream-border rounded text-ink-muted">
                  Đã chọn {selectedStudentIds.size}
                </span>
              </div>
              
              {students.length === 0 ? (
                <p className="text-sm text-ink-muted italic">Chưa có học sinh nào trong hệ thống.</p>
              ) : (
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {students.map(student => (
                    <div 
                      ...
                      className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-colors border ${selectedStudentIds.has(student.id) ? 'bg-white border-primary shadow-sm' : 'border-transparent hover:bg-cream-border'}`}
                    >
                      <div className="text-primary shrink-0">
                        {selectedStudentIds.has(student.id) ? <CheckSquare size={20} /> : <Square size={20} className="text-cream-border" />}
                      </div>
                      <img 
          ...
          <div className="w-full md:w-2/3 flex flex-col min-h-0 border-l border-cream-border pl-6">
            <div className="flex justify-between items-center mb-4 shrink-0">
              <div>
                <h4 className="font-bold text-ink">Ngân hàng câu hỏi ({questions.length} câu)</h4>
                <p className="text-sm text-ink-muted">Đã chọn: {selectedIds.size} câu</p>
              </div>
              <div className="flex gap-2">
                <button 
                  onClick={() => selectRandom(5)}
                  className="px-3 py-1.5 bg-gold-100 text-gold-600 border border-gold-100 rounded-lg hover:opacity-80 transition-colors font-medium text-sm flex items-center gap-1"
                >
            ...
            <div className="flex-1 overflow-y-auto border border-cream-border rounded-2xl bg-cream p-2 space-y-2">
              {fetching ? (
                <p className="text-center py-8 text-ink-muted">Đang tải câu hỏi...</p>
              ) : questions.length === 0 ? (
                <p className="text-center py-8 text-ink-muted">Chưa có câu hỏi nào. Hãy nhập CSV!</p>
              ) : (
                questions.map(q => {
                  let text = 'N/A';
                  try {
                  ...
                  return (
                    <div 
                      ...
                      className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer transition-all border ${
                        isSelected ? 'bg-white border-primary shadow-sm' : 'bg-white border-transparent hover:border-cream-border'
                      }`}
                    >
                      <div className="text-primary shrink-0">
                        {isSelected ? <CheckSquare size={20} /> : <Square size={20} className="text-cream-border" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-ink text-sm truncate">{text}</p>
                      </div>
                      <div className="text-xs font-medium px-2 py-1 rounded bg-cream-border text-ink-muted shrink-0">
                        Mức {q.level}
                      </div>
                    </div>
                  )
        ...
        <div className="p-6 border-t border-cream-border flex justify-end gap-4 bg-white shrink-0">
          <button onClick={onClose} className="px-6 py-2 border border-cream-border text-ink rounded-xl hover:bg-cream font-medium transition-colors">
            Hủy
          </button>
          <button 
            onClick={handleSave}
